...
function fg_register_metabox() {
	...
	$post_types	= apply_filters( 'fg_post_types', array( 'services', 'events' ) );
	$context	= apply_filters( 'fg_context', 'normal' );
	$priority	= apply_filters( 'fg_priority', 'high' );

	foreach ( $post_types as $post_type ) {

		add_meta_box( 'featuredgallerydiv', __( 'Galería de Imágenes', 'featured-gallery' ), 'fg_display_metabox', $post_type, $context, $priority );
...
function fg_display_metabox() {
	...
	if ( ! empty( $galleryString ) ) {
		foreach ( $galleryArray as &$id ) {
			$image = wp_get_attachment_image_src( $id, array( 150, 150 ) );
